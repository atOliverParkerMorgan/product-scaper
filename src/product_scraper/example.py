...
if __name__ == "__main__":
    # try:
    #     product_scraper = ProductScraper(categories=CATEGORIES, websites_urls=[])
    ...
    # print(product_scraper.predict(["https://www.valentinska.cz/home"]))
    # product_scraper.save()

    CATEGORIES = ["image", "title", "price"]
    WEBSITES = [
        "https://www.morganbooks.eu/"
    ]  # example site use more site ideally 50+ for good prediction results...
    product_scraper = ProductScraper(categories=CATEGORIES, websites_urls=WEBSITES)

    # then you call to create trianing data for the random forest model or jsut for plain xpath extraction
    product_scraper.create_training_data()
    # img https://imgur.com/a/0A52xw9
